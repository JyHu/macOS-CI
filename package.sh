...
do
    case $opt in
        ...
        h)
            ...
            echo ""
            echo "eg: sh package.sh -c Release -a"
            exit 0;;
        ?)
            echo "unknown"
            exit 1;;
...
if [[ "$__WORKING_CONFIGURATION__" == "" ]]; then
    LOG "请选择需要打包的环境，如 Release Beta 等"
    exit 1
fi
...
# 以Mac根目录为根节点创建目录
function makeHomeFolder() {
    if [[ -d "/Users/`whoami`/$1" ]]; then
        LOG "存在重复的目录并移除 ~/$1"
        rm -rf ~/$1
    fi
...
#脚本所在目录
__SCRIPT_FOLDER__=$(dirname $0)
__PROJECT_FOLDER__=`pwd`

LOG "脚本目录：" $__SCRIPT_FOLDER__
LOG "__PROJECT_FOLDER__：" $__PROJECT_FOLDER__
...
echo "Package options: $__PACK_OPTIONS_FILE__"
...
LOG "解析设备、账号信息:" "$__PACK_OPTIONS_FILE__"
LOG "  Project name: $__PROJ_NAME__"
LOG "  Mac password: $__MAC_PWD__"
LOG "  APP Version : $__BUILD_PROJ_VER__"
...
function build_package() {
    ...
    LOG "*********************************************************************************"
    LOG ""
    LOG "开始打包 (Target: $1, Configuration: $__WORKING_CONFIGURATION__):"
    LOG "  Bundle ID: $__BUNDLE_ID__"
    LOG "  Certficate: $__BUILDING_CERT__"
    LOG "  Team ID: $__TEAM_ID__"
    LOG "  Scheme: $__BUILDING_SCHEME__"
    ...
    # 创建打包目录，每个scheme单独创建
    mkdir -p $__BUILD_PATH__/$__BUILDING_SCHEME__

    if [[ "$__WORKING_CONFIGURATION__" == "RC" ]]; then
    ...
    # 清理资源
    LOG "清理项目资源 ..." "xcodebuild clean -workspace $__PROJ_NAME__ -scheme $__BUILDING_SCHEME__ -configuration $__XCCONFIGURATION__"
    xcodebuild clean -workspace $__PROJ_NAME__ \
        -scheme $__BUILDING_SCHEME__ \
        -configuration $__XCCONFIGURATION__

    # 编译项目
    LOG "编译项目" "xcodebuild archive -quiet -workspace $__PROJ_NAME__ -scheme $__BUILDING_SCHEME__ -configuration $__XCCONFIGURATION__ -archivePath $__BUILD_PATH__/$__BUILDING_SCHEME__ -destination generic/platform=macOS"
    xcodebuild archive -quiet \
        -workspace $__PROJ_NAME__ \
        -scheme $__BUILDING_SCHEME__ \
        -configuration $__XCCONFIGURATION__ \
        -archivePath $__BUILD_PATH__/$__BUILDING_SCHEME__ \
        -destination generic/platform=macOS
    
    LOG "编译完成 (Target: $1, Configuration: $__WORKING_CONFIGURATION__)"

    #-------------------------------------------------------------------------------------
    #----------  查找编译文件，如果编译成功了就继续，否则退出后续操作

    # 查找 archive 包
    __ARCHIVE_FILE_PATH__="$__BUILD_PATH__/$__BUILDING_SCHEME__.xcarchive"
    ...
    # 导出app的信息
    __EXPORT_OPTIONS_PLIST__="$__BUILD_PATH__/$__BUILDING_SCHEME__/exportOptions.plist"
...
EOF
    ...
    # 导出打包的app
    LOG "导出app文件：" "xcodebuild -exportArchive -archivePath $__ARCHIVE_FILE_PATH__ -exportPath \"$__BUILD_PATH__/$__BUILDING_SCHEME__\" -exportOptionsPlist \"$__EXPORT_OPTIONS_PLIST__\""
    xcodebuild -exportArchive \
        -archivePath $__ARCHIVE_FILE_PATH__ \
        -exportPath "$__BUILD_PATH__/$__BUILDING_SCHEME__" \
        -exportOptionsPlist "$__EXPORT_OPTIONS_PLIST__"

    # 获取应用程序的包名
    __ARCHIVED_APP_NAME__=`ls "$__BUILD_PATH__/$__BUILDING_SCHEME__" | grep .*app$`
    __ARCHIVED_APP_PATH__="$__BUILD_PATH__/$__BUILDING_SCHEME__/$__ARCHIVED_APP_NAME__"
...
#        spctl --verbose=4 --assess --type execute "$__ARCHIVED_APP_PATH__"
        ...
        __ZIPED_FILE__="$__BUILD_PATH__/$__BUILDING_SCHEME__/singed_$__ARCHIVED_APP_NAME__.zip"
        ...
        # 查询公正信息的文件
        __NOTARIZE_INFO_PLIST__="$__BUILD_PATH__/$__BUILDING_SCHEME__/notarize_info.plist"
        ...
        if [[ "$__NOTARIZE_STATUS__" =~ "Accepted" ]]; then
            ...
            LOG "解压公证过的zip后移除" "unzip -q -d \"$__BUILD_PATH__/$__BUILDING_SCHEME__\" \"$__ZIPED_FILE__\" && rm \"$__ZIPED_FILE__\""
            unzip -q -d "$__BUILD_PATH__/$__BUILDING_SCHEME__" "$__ZIPED_FILE__" && rm "$__ZIPED_FILE__"

            LOG "对app添加票据" "xcrun stapler staple -v \"$__BUILD_PATH__/$__BUILDING_SCHEME__/$__ARCHIVED_APP_NAME__\""
            xcrun stapler staple "$__BUILD_PATH__/$__BUILDING_SCHEME__/$__ARCHIVED_APP_NAME__"

            __APP_INFO_PLIST__="./$__BUILD_PATH__/$__BUILDING_SCHEME__/$__ARCHIVED_APP_NAME__/Contents/Info.plist"
            ...
            cat > $__BUILD_PATH__/$__BUILDING_SCHEME__/appdmg.json <<- EOF
{
...
EOF
            ...
            LOG "写入dmg打包配置" "$__BUILD_PATH__/$__BUILDING_SCHEME__/appdmg.json"
            cat "$__BUILD_PATH__/$__BUILDING_SCHEME__/appdmg.json"

            LOG "增加dmg背景资源 ${__BUILDING_TARGET__}.png"
            cp "./${__SCRIPT_FOLDER__}/background/${__BUILDING_TARGET__}.png" "$__BUILD_PATH__/$__BUILDING_SCHEME__/background.png"

            # 拼接打包的 configuration
            if [[ "$__WORKING_CONFIGURATION__" == "RC" ]]; then
                __DISPLAY_CONFIGURATION__="RC_"
            elif [[ "$__WORKING_CONFIGURATION__" == "Release" ]]; then
                __DISPLAY_CONFIGURATION__=""
            else
                __DISPLAY_CONFIGURATION__=$__WORKING_CONFIGURATION__
            fi

            __DMG_NAME__="${__DMG_CONFIG_FILE_NAME__}_${__DISPLAY_CONFIGURATION__}${__APP_SHORT_VER__}_${__APP_BUILD_VER__}"

            LOG "制作DMG文件" "appdmg $__BUILD_PATH__/$__BUILDING_SCHEME__/appdmg.json $__ARCHIVE_DMG_FOLDER__/$__DMG_NAME__.dmg"
            appdmg $__BUILD_PATH__/$__BUILDING_SCHEME__/appdmg.json $__ARCHIVE_DMG_FOLDER__/$__DMG_NAME__.dmg
    ...
    # 移除一下归档文件
    rm -rf $__BUILD_PATH__/*.xcarchive
}
...
APP_VER=$(sed -n '/MARKETING_VERSION/{s/MARKETING_VERSION = //;s/;//;s/^[[:space:]]*//;p;q;}' `ls ./ | grep .*xcodeproj`/project.pbxproj)
LOG "处理打包结果。。。。"

# 如果有打包目录，才算是打包成功
if [ -d "./$WORKING_PATH" ]; then
    # 当前打包结果移动到的目录
    DESTINATION_DMG="Shared/Packages/${__WORKING_CONFIGURATION__}/${APP_VER}"

    # 移除旧的打包资源
    makeHomeFolder $DESTINATION_DMG
    ...
    if [[ "${__WORKING_CONFIGURATION__}" == "RC" ]]; then
        # dSYM目录
        DESTINATION_DSYM="Shared/Performance/dSYMs/RC/${APP_VER}"
    else
        # dSYM目录
        DESTINATION_DSYM="Shared/Performance/dSYMs/${APP_VER}"
    fi

    makeHomeFolder $DESTINATION_DSYM
